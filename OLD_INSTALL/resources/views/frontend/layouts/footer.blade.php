						...
						<div class="social-icon">
							@if($socialLinks->count() > 0)
								@foreach($socialLinks as $socialLink)
									<a href="{{ $socialLink->description->social_link }}" target="_blank" class="icon">
										<i class="{{ $socialLink->description->social_icon }}"></i>
									</a>
								@endforeach
							@endif
						</div>
					...
					<ul>
						@isset($contentDetails['extra-pages'])
							@foreach($contentDetails['extra-pages'] as $data)
								<li>
									<a href="{{route('getLink', [$data->content_id, Str::slug($data->description->title)])}}">
							...
							@foreach($contactDetails as $contactDetail)
								<li>
									<div class="d-flex flex-row align-items-center">
										<div class="icon">
											<img
												src="{{ getFile(config('location.content.path').(isset($contactDetail->content->contentMedia->description->image) ? $contactDetail->content->contentMedia->description->image : '')) }}">
										</div>
										<div>
											<p class="contant-details">{!! $contactDetail->description->short_description !!}</p>
										</div>
									</div>
								</li>
							@endforeach
