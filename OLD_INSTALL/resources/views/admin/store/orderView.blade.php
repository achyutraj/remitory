														...
														@foreach($orderDetails as $key => $item)
															<tr>
																...
																<td data-label="@lang('Attributes')">@foreach($item->attr as $attr)
																	...
																	@endforeach</td>
																<td data-label="@lang('Price')">{{optional(auth()->user()->storeCurrency)->symbol}}{{ number_format($item->price,2) }}</td>
																<td data-label="@lang('Quantity')">{{ $item->quantity }}</td>
																<td data-label="@lang('Total Price')"><span
																		class="text-dark font-weight-bold">{{optional(auth()->user()->storeCurrency)->symbol}}{{ number_format($item->total_price,2) }}</span>
																</td>
															</tr>
														@endforeach
														</tbody>
										...
										<div class="card mb-4 card-primary shadow">
											...
											<div class="card-body">
												...
												<li class="list-group-item d-flex justify-content-between">
													...
													<span
														class="text-info">{{number_format($order->total_amount,2)}} {{optional(auth()->user()->storeCurrency)->code}}</span>
												</li>
												@if($order->shipping_charge > 0)
													<li class="list-group-item d-flex justify-content-between">
														<span>{{ __('Shipping Charge') }}</span>
														<span
															class="text-danger">{{number_format($order->shipping_charge,2)}} {{optional(auth()->user()->storeCurrency)->code}}</span>
													</li>
												@endif
												<li class="list-group-item d-flex justify-content-between">
													<span>{{ __('Total Amount') }}</span>
													<span
														class="text-dark font-weight-bold">{{number_format($order->total_amount+$order->shipping_charge,2)}} {{optional(auth()->user()->storeCurrency)->code}}</span>
												</li>
												<li class="list-group-item d-flex justify-content-between">
