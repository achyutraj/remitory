						...
						<div class="section__title about-content">
							<div class="row">
								...
								<div class="col-md-12 mb-5">
									<span class="section__cate">@lang(optional($aboutUsContent->description)->title)</span>
			...
			<div class="container">
				...
				<div class="app-services-block">
					...
					@foreach($featureContents as $key => $featureContent)
						@if($key%2==0)
							<div class="app-services-point">
								<img src="{{ getFile(config('location.content.path').(isset(optional(optional(optional($featureContent->content)->contentMedia)->description)->image) ? optional(optional(optional($featureContent->content)->contentMedia)->description)->image : '')) }}">
					...
					<div class="app-services-inner middle-img">
						<img src="{{ getFile(config('location.template.path').(isset($feature->templateMedia()->description->image) ? $feature->templateMedia()->description->image : '')) }}" alt="@lang(optional($feature->description)->title)">
					</div>
					...
					<div class="col-xl-3 col-sm-6">
						<div class="counter-item">
							...
							<div class="counter-content">
								...
								<div class="counter-header">
									<h5 class="subtitle"><sup>{{ __(basicControl()->currency_symbol) }}</sup>@lang(optional($customersContent->description)->total_currency)</h5>
								</div>
							</div>
						</div>
					</div>
					<div class="col-xl-3 col-sm-6">
						<div class="counter-item">
							...
							<div class="counter-content">
								...
								<div class="counter-header">
									<h5 class="subtitle"><sup>{{ __(basicControl()->currency_symbol) }}</sup>@lang(optional($customersContent->description)->total_wallet)</h5>
								</div>
							</div>
						</div>
					</div>
					...
					@foreach($faqContents as $faqContent)
						<div class="faq-block-item {{ $loop->first ? 'active open' : '' }}">
							<div class="faq-block-title">
								<h5 class="title"><i class="las la-question"></i>@lang(optional($faqContent->description)->title)
								</h5>
							</div>
