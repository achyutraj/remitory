						...
						@if(isset($contentDetails))
							@foreach($contentDetails as $contentDetail)
								<li class="contact-list-item py-3">
									<span class="icon">
										<img
											src="{{ getFile(config('location.content.path').(isset($contentDetail->content->contentMedia->description->image) ? $contentDetail->content->contentMedia->description->image : '')) }}">
									</span>
									<span
